...
func (p *Player) Update(g *Game, delta float64) {
	...
	/*fmt.Println("Name : " ,s.Player.Name )
	fmt.Println("direction : ",s.Player.Direction)
	fmt.Println("marker : ",s.Player.Marker)
	fmt.Println("color : ",s.Player.Color)
	fmt.Println("pos : ",s.Player.Pos )
	fmt.Println("score : ",s.Player.score )
	fmt.Println("trail : ",s.Player.Trail)
...
func (gm *GameManager) SendServerNewGame(){
	g := NewGame(gameWidth, gameHeight)
	gm.Games[g.Id]=g
	go g.Run()
}

/*func (gm *GameManager) InputGameInformation(){
...
func (gm *GameManager) HandleSavedChannel(c ssh.Channel, color string,player *Player) {
	...
	//colorOptions := g.AvailableColors()
	//finalColor := colorOptions[0]
	finalColor := player.Color

	session := NewSession(c, g.WorldWidth(), g.WorldHeight(), finalColor)
	g.AddSession(session)

	session.Player.Name = player.Name
	session.Player.Direction = player.Direction
	session.Player.Marker = player.Marker
	session.Player.Color = player.Color
	session.Player.Pos.X = player.Pos.X
	session.Player.Pos.Y = player.Pos.Y
	session.Player.score = player.score
	session.Player.Trail = player.Trail

	/*fmt.Println("Name : " ,session.Player.Name )
	fmt.Println("direction : ",session.Player.Direction)
	fmt.Println("marker : ",session.Player.Marker)
	fmt.Println("color : ",session.Player.Color)
	fmt.Println("pos : ",session.Player.Pos )
	fmt.Println("score : ",session.Player.score )
	fmt.Println("trail : ",session.Player.Trail)*/
...
func (g *Game) worldString(s *Session) string {
	...
	for player := range g.players() {
		...
		pos := player.Pos
		colorizer := color.New(player.Color).SprintFunc()
		//fmt.Printf("x : %d , y : %d\n",(pos.RoundX()+1),(pos.RoundY()+1))
		strWorld[pos.RoundX()][pos.RoundY()] = colorizer(string(player.Marker))

		// Load the player's trail into the rune slice
		for _, segment := range player.Trail {
			x, y := segment.Pos.RoundX(), segment.Pos.RoundY()
...
func (g *Game) Run() {
	...
	go func() {
		...
		for now := range c {
			...
			delta := float64(now.Sub(lastUpdate)) / float64(time.Millisecond)
...
func (g *Game) Update(delta float64) {
	...
	/*	fmt.Println("Name : " ,player.Name )
		fmt.Println("direction : ",player.Direction)
		fmt.Println("marker : ",player.Marker)
		fmt.Println("color : ",player.Color)
		fmt.Println("pos : ",player.Pos )
		fmt.Println("score : ",player.score )
		fmt.Println("trail : ",player.Trail)*/
	//time.Sleep(10*time.Second)
		...
		if pos.RoundX() < 0 || pos.RoundX() >= len(g.level) ||
			...
			fmt.Println("number 1 startover")
		}
	...
	for player, session := range g.players() {
		...
		if segment, collided := trailCoordMap[playerPos]; collided {
			...
			session.StartOver(g.WorldWidth(), g.WorldHeight())
			fmt.Println("number 2 startover")
		}
	}
}
...
func (s *Session) StartOver(worldWidth, worldHeight int) {
	fmt.Println("end game now "+s.Player.Name)
